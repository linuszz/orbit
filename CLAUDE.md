...
|---|---|---|---|
| Session | `Space` (`SpaceId`) | **Orbit** / Constellation | `orbt dev` attaches to dev |
| Daemon / Server | `orbtd` binary | **Core** | the space station |
| Client / TUI | `orbt` binary | **Ground Station** | the CLI binary |
| Window | (within `Space`) | **Deck** | `orbt deck list` |
| Pane | `Pane` (`PaneId`) | **Port** | `orbt port split h` |
| PTY | (within `Pane`) | — | the observation aperture |
| Agent | `Agent` (`AgentId`) | **Satellite** | `orbt sat list` |
...
| SSH attach | — | **Link-up** | `orbt --remote user@host` |
| Detach | — | **Go Dark** | session persists without client |
| Clipboard sync | (Beacon channel) | **Beacon** | `orbt beacon sync` |
| Image transfer | (Downlink channel) | **Downlink** | `orbt downlink last` |
| File transfer | (Payload channel) | **Payload** | `orbt payload up file.txt` |
| Command Palette | `Overlay::CommandPalette` | **Flight Deck** | triggered by prefix key |
| Plugin | `OrbitPlugin` trait | **Module** | `orbt module list` |
| MCP tool | — | **Instrument** | `~/.orbt/instruments/` |
...
(`orbt dev` ≈ `ssh dev`), and a complete aerospace metaphor ecosystem to
...
|---|---|---|---|
| 1 | **Mercury** | Session/Pane TUI + PTY + IPC + SSH attach | orbt, orbtd, orbt-protocol, orbt-core |
...
| 4 | **Mars** | WASM plugin system (wasmtime), MCP integration | + orbt-plugin |
...
```
orbt/                              # this repo — implementation only
...
├── crates/
│   ├── orbt/                      # BIN: TUI client (Ground Station)
│   │   └── src/
...
│   ├── orbt-protocol/             # LIB: shared wire types (IPC contract)
│   │   └── src/{lib,messages,types,encoding,error}.rs
│   └── orbt-core/                 # LIB: domain model + VT emulation (no tokio)
...
```
orbt (client / Ground Station)        orbtd (daemon / Core)
...
- **Client never touches PTY.** All PTYs are owned by `orbtd`. Client only
...
|---|---|---|---|
| `orbt-protocol` | lib | No | `ClientMessage`, `ServerEvent`, `Capabilities`, `Cell`, `CellGrid`, `TermColor`, ID newtypes, `ProtocolError` |
| `orbt-core` | lib | **No** (pure sync) | `Config`, `VtParser`, `CellGrid` ops, `VtError`/`GridError` |
| `orbtd` | bin | Yes | session/pty/agent/io/ipc modules |
| `orbt` | bin | Yes | app state, tui (render/layout/theme/widgets), events (keyboard/mouse/drag), ipc client, settings persistence |

`orbt-protocol` and `orbt-core` MUST remain tokio-free so they can be unit
tested in isolation.
...
RGB constants live in `crates/orbt/src/tui/theme.rs` as `ThemeColors::orbt()`.
The theme system supports runtime switching via `set_theme("orbt"|"tokyo-night")`
...
- **Override**: `ORBT_PREFIX_KEY=ctrl+space` env var, or
  `prefix_key = "ctrl+b"` in `~/.orbt/config.toml` `[general]`
...
These are already defined in `orbt-protocol` (the IPC contract). Do not
diverge.
...
- **Max message size**: 4 MB (`MAX_MSG_BYTES` constant in
  `orbt-protocol::encoding`). Reject larger — defends against OOM.
...
Full variant set in `crates/orbt-protocol/src/messages.rs`. Treat as the
...
|---|---|
| `orbt-protocol` | `thiserror` — `ProtocolError` enum (callers can `match` on `VersionMismatch`/`MessageTooLarge`/etc.) |
| `orbt-core` | `thiserror` — `VtError`, `GridError` enums |
| `orbtd` (binary) | `anyhow::Result` in `main`, propagates to exit code + log |
| `orbt` (binary) | `anyhow::Result` in `main` |
...
```rust
// Priority: $XDG_RUNTIME_DIR/orbt.sock → $TMPDIR/orbt-{uid}.sock
...
- Grid ops: hand-rolled in `orbt-core::vt::grid` (we control `Cell` size)
...
`orbtd` PTY read loop batches reads within an 8 ms window before emitting
...
`ORBT_LOG_LEVEL` env var (default `info` for orbtd, `warn` for orbt).
Logs to `~/.orbt/logs/orbitd-<date>.log`. Never log in the PTY I/O hot
...
| Var | Effect |
|---|---|
| `ORBT_CONFIG_DIR` | Override `~/.orbt/` |
| `ORBT_DEFAULT_CONSTELLATION` | Default session name |
| `ORBT_PREFIX_KEY` | Override prefix key (e.g. `ctrl+space`) |
| `ORBT_MOBILE` | Force mobile TUI mode (Phase 4+) |
| `ORBT_NO_BEACON` | Disable clipboard bridge |
| `ORBT_LOG_LEVEL` | tracing filter (e.g. `debug`, `orbitd=trace`) |
| `ORBT_INSTRUMENTS` | Comma-separated MCP server names to enable (Phase 4) |
| `ORBT_PANE_ID` | **Set by orbitd on every PTY child** — agent detection reads this |
...
Located at `$XDG_CONFIG_HOME/orbt/settings.toml` (or `~/.config/orbt/settings.toml`).
...
4. **Cell grid clone on every PTY read** in `orbtd` (snapshot prep). ~160
...
| **GAP 3**: daemon lifecycle | ⚠ Open | Add signal handling (SIGTERM/SIGINT → graceful PTY shutdown → unlink socket), PID/lock file at `$XDG_RUNTIME_DIR/orbt.lock`, `prctl(PR_SET_PDEATHSIG)` for PTY children. |
| **GAP 4**: error layering | ✅ Done | `thiserror` in `orbt-protocol`/`orbt-core`; `anyhow` in `orbt`/`orbtd` mains. |
...
- ✅ Settings persistence: `~/.config/orbt/settings.toml` (theme, sidebar, panel)
...
- ✅ Orange color system canonical (encoded in `ThemeColors::orbt()`)
...
- **Library crates stay tokio-free** (`orbt-protocol`, `orbt-core`)
...
| What's the wire format / message variant? | `crates/orbt-protocol/src/messages.rs` (canonical) or `02_design/06_tech-design/03-ipc-protocol.md` |
...
| What theme tokens / colors exist? | `crates/orbt/src/tui/theme.rs` (`ThemeColors` struct + accessor fns) |
| How does settings persistence work? | `crates/orbt/src/app.rs` (`UserSettings`, `load_settings`, `save_settings`) |
...
- **Ground Station** — the `orbt` TUI client
- **Instrument** — MCP tool
- **Link-up** — SSH attach
