...
pub fn config_dir() -> std::path::PathBuf {
    if let Ok(dir) = std::env::var("ORBT_CONFIG_DIR") {
        return std::path::PathBuf::from(dir);
    }
    if let Ok(home) = std::env::var("HOME") {
        return std::path::PathBuf::from(home).join(".orbt");
    }
    std::path::PathBuf::from(".orbt")
}
